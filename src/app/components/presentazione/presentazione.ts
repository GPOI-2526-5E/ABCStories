...
export class Presentazione implements AfterViewInit {
  ...
  @ViewChild('communityScroll') communityScroll!: ElementRef; // ← NUOVO
  ...
  showLeftGenre = false;
  showRightGenre = true;
  activeDot = 0; // ← NUOVO: dot attivo per le community card
  ...
  ngAfterViewInit() {
    ...
    revealSections.forEach((section, index) => {
      ...
      gsap.set(items, { opacity: 0, x: direction, y: 40, filter: 'blur(10px)' });

      if (button) gsap.set(button, { opacity: 0, scale: 0.85, y: 30 });
    ...
    if (genreSection) {
      ...
      gsap.set(genreElements, { opacity: 0, y: 60, filter: 'blur(8px)' });
      gsap.set(genreCards, { opacity: 0, y: 50, scale: 0.92 });
      ...
      tlGenres.to(genreElements, {
        ...
        duration: 0.8, ease: 'power3.out', stagger: 0.14,
      });
    ...
    sections.forEach((section, index) => {
      ...
      gsap.set(card, { opacity: 0, x: direction, y: 40, scale: 0.96 });
      gsap.set(cardItems, { opacity: 0, y: 25, filter: 'blur(8px)' });
      gsap.set(image, { opacity: 0, x: imageDirection, y: 35, scale: 1.04, rotate: index % 2 === 0 ? 1.5 : -1.5 });
      ...
      tl.to(card, { opacity: 1, x: 0, y: 0, scale: 1, duration: 0.75, ease: 'power3.out' });
      tl.to(cardItems, { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.65, ease: 'power3.out', stagger: 0.12 }, '-=0.5');
      tl.to(image, { opacity: 1, x: 0, y: 0, scale: 1, rotate: 0, duration: 0.85, ease: 'power3.out' }, '-=0.6');
    });
    ...
    // ANIMAZIONE CONT5 — intro community
    ...
    if (communityIntro) {
      ...
      gsap.set(introItems, { opacity: 0, y: 50, filter: 'blur(10px)' });
      if (introButton) gsap.set(introButton, { opacity: 0, y: 30, scale: 0.9 });
    ...
    if (communityTitle) {
      gsap.set(communityTitle, { opacity: 0, y: 60, filter: 'blur(8px)' });
    ...
    // ANIMAZIONE CONT5 — community card
    ...
    communityCards.forEach((card, index) => {
      ...
      gsap.set(card, { opacity: 0, x: direction, y: 40, scale: 0.96 });
      gsap.set(items, { opacity: 0, y: 25, filter: 'blur(8px)' });
      ...
      tlCard.to(card, { opacity: 1, x: 0, y: 0, scale: 1, duration: 0.75, ease: 'power3.out' });
      tlCard.to(items, { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.65, ease: 'power3.out', stagger: 0.12 }, '-=0.5');
    });

    // ANIMAZIONE family section
    ...
    if (familySection) {
      ...
      gsap.set(familyItems, { opacity: 0, y: 65, filter: 'blur(10px)' });
      if (familyButton) gsap.set(familyButton, { opacity: 0, y: 30, scale: 0.88 });
      ...
      tlFamily.to(familyItems, {
        opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.9, ease: 'power3.out', stagger: 0.16,
      });
  ...
  // Aggiorna il dot attivo in base alla card visibile

  onCommunityScroll() {
    if (!this.communityScroll) return;
    const el = this.communityScroll.nativeElement as HTMLElement;
    const cardWidth = el.clientWidth;
    this.activeDot = Math.round(el.scrollLeft / cardWidth);
  }
}